...
class ModelTestCase(AllenNlpTestCase):
    ...
    def set_up_mtl_model(self, param_file, dataset_files: Dict[str, str]):
        ...
        train_datas = {name: reader.read(dataset_files[name])
                       for name, reader in dataset_readers.items()}
        instances = (instance for dataset in train_datas.values() for instance in dataset)
        # The dataset reader might be lazy, but a lazy list here breaks some of our tests.
        # instances = list(reader.read(str(dataset_file)))
        # Use parameters for vocabulary if they are present in the config file, so that choices like
        # "non_padded_namespaces", "min_count" etc. can be set if needed.
    ...
    def ensure_mtl_model_can_train_save_and_load(self,
                                             param_file: str,
                                             tolerance: float = 1e-4,
                                             cuda_device: int = -1,
                                             gradients_to_ignore: Set[str] = None,
        ...
        mingler = DatasetMingler.from_params(params["mingler"])
        dataset_reader_params = params.pop('dataset_readers')
        ...
        # We'll check that even if we index the dataset with each model separately, we still get
        # the same result out.
        model_dataset = {name: reader.read(params['validation_data_paths'][name])
                       for name, reader in dataset_readers.items()}

        # model_dataset = reader.read(params['validation_data_path'])
        iterator.index_with(model.vocab)
        model_batch = next(iterator(list(mingler.mingle(model_dataset)), shuffle=False))

        loaded_dataset = {name: reader.read(params['validation_data_paths'][name])
                         for name, reader in dataset_readers.items()}

        # loaded_dataset = reader.read(params['validation_data_path'])
        iterator2.index_with(loaded_model.vocab)
        loaded_batch = next(iterator2(list(mingler.mingle(loaded_dataset)), shuffle=False))

        # Check gradients are None for non-trainable parameters and check that
        # trainable parameters receive some gradient if they are trainable.
        ## not doing this, because if the instance is just specific to one task, the other task-specific
        ## parameter must not receive any gradients.
